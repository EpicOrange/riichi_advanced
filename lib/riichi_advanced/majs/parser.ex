  ...
  def parse(input) when is_binary(input) do
    case byte_size(input) do
      ...
      _ ->
        ...
        |> Code.string_to_quoted(emit_warnings: false, columns: true, existing_atoms_only: true, static_atoms_encoder: fn name, _pos -> {:ok, if name == "do" do :do else name end} end)
        |> case do
          {:ok, ast} -> {:ok, ast}
          {:error, err} -> {:error, err}
        end

...
defmodule RiichiAdvanced.Validator do
  ...
  def validate_event_name(name) when is_binary(name) do
    name in @allowed_events
  end
  def validate_event_name(_name), do: false

  @valid_path_regex ~r/^(\.[a-zA-Z0-9_]+|\[-?[0-9]+\]|\["[a-zA-Z0-9_ ]+"\])*(\[-?[0-9]+(,\s*-?[0-9]+)*\]|\["[a-zA-Z0-9_ ]+"(,\s*"[a-zA-Z0-9_ ]+")*\])?(\.[a-zA-Z0-9_]+|\[-?[0-9]+\]|\["[a-zA-Z0-9_ ]+"\])*$/

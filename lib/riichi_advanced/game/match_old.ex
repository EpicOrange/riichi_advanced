defmodule RiichiAdvanced.MatchOld do
    ...
    cond do
      is_list(group) ->
        ...
        hand_calls = for subgroup <- group_to_subgroups(nojoker, base_tile, tile_behavior), reduce: [{hand, calls}] do
          hand_calls -> Enum.flat_map(hand_calls, fn {hand, calls} -> remove_from_hand_calls(hand, calls, subgroup, %{ tile_behavior | aliases: %{}, mappings: %{}, uuid: Ecto.UUID.generate() }) end)
        end
  ...
  def collect_base_tiles(hand, calls, offsets, tile_behavior \\ %TileBehavior{}) do
    ...
    mappings = for {tile, mappings} <- tile_behavior.mappings, base_tile <- base_tiles, Utils.same_tile(base_tile, tile) do mappings end
  ...
  # @decorate cacheable(cache: RiichiAdvanced.Cache.Memo, key: {:remove_match_definition, hand, calls, match_definition, TileBehavior.hash(tile_behavior)})
      ...
      hand_calls ->
        ...
        case match_definition_elem do
          ...
          [groups, num] ->
            ...
            tile_behavior = if nojoker do %{ tile_behavior | aliases: %{}, mappings: %{}, uuid: Ecto.UUID.generate() } else tile_behavior end
            new_hand_calls = if unique and num >= 1 and not exhaustive and Enum.all?(groups, &not is_list(&1) and (Utils.is_tile(&1) or &1 in @group_keywords)) do
              ...
              Enum.flat_map(hand_calls, fn {hand, calls} ->
                ...
                {[hand | calls], _, _, to_remove_num} = for {call, is_hand} <- Enum.map(calls, &{&1, false}) ++ [{hand, true}], reduce: {[], joker, nojoker, num} do
                  {ret, joker, nojoker, to_remove_num} ->
                    ...
                    adj_nojoker = Map.new(Enum.with_index(nojoker), fn {tile, i} -> {length(joker) + i, for {tile2, j}  <- Enum.with_index(tiles), Utils.same_tile(tile2, tile, %{ tile_behavior | aliases: %{}, mappings: %{}, uuid: Ecto.UUID.generate() }) do j end} end)
              ...
              tile_behavior = if no_joker_index != nil and i > no_joker_index do %{ tile_behavior | aliases: %{}, mappings: %{}, uuid: Ecto.UUID.generate() } else tile_behavior end
  ...
  # @decorate cacheable(cache: RiichiAdvanced.Cache.Memo, key: {:match_hand, hand, calls, match_definitions, TileBehavior.hash(tile_behavior)})
  ...
  def binary_search_count_matches(hand_calls, match_definitions, tile_behavior, l \\ -1, r \\ 1) do
    if l < r do
      ...
      if Enum.empty?(multiplied_match_def) do
        ...
        matched = Enum.any?(hand_calls, fn {hand, calls} -> RiichiAdvanced.Match.match_hand(hand, calls, multiplied_match_def, tile_behavior) end)
        {l, r} = if matched do

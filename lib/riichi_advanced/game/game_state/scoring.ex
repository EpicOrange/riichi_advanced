...
defmodule RiichiAdvanced.GameState.Scoring do
  ...
  def get_minipoints(state, seat, winning_tile, win_source) do
    ...
    Riichi.calculate_fu(state.players[seat].hand, state.players[seat].calls, winning_tile, win_source, Riichi.get_seat_wind(state.kyoku, seat), Riichi.get_round_wind(state.kyoku), state.players[seat].tile_ordering, state.players[seat].tile_ordering_r, state.players[seat].tile_aliases, enable_kontsu_fu)
  end
  ...
  def adjudicate_win_scoring(state) do
    ...
    sanchahou = Map.get(score_rules, "triple_ron_draw", false) && map_size(winners) == 3
  ...
  def calculate_winner_details(state, seat, possible_winning_tiles, win_source, get_worst_yaku \\ false) do
    ...
    %{
      ...
      win_source: win_source,
      yaku: if yaku_2_overrides do [] else yaku end,
      yaku2: yaku2,
      points: points,
      points2: points2,
      score: score,

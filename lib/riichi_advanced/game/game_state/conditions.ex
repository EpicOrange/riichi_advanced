...
defmodule RiichiAdvanced.GameState.Conditions do
  ...
  def check_condition(state, cond_spec, context \\ %{}, opts \\ []) do
    ...
    result = case cond_spec do
      ...
      "call_available"              -> last_action != nil and (last_action.action == :discard or last_action.action == :call) and Riichi.can_call?(context.calls_spec, cxt_player.hand, cxt_player.tile_behavior, [Map.get(last_action, :tile, Map.get(last_action, :called_tile, nil))])
      "self_call_available"         -> Riichi.can_call?(context.calls_spec, cxt_player.hand ++ cxt_player.draw, cxt_player.tile_behavior, [])
      ...
      "has_points"              ->
        totals = context.existing_yaku
        |> Enum.map(fn {_name, value} -> value end)
        |> Enum.reduce([], &Scoring.add_yaku_values/2)
        denomination = Enum.at(opts, 1, nil)
        if denomination == nil do
          # just take the first denomination
          case totals do
            [] -> 0
            [points | _rest] -> points
      ...
      "call_would_change_waits" ->
        ...
        waits = Match.get_waits(hand, calls, win_definitions, cxt_player.tile_behavior)
        Enum.all?(Riichi.make_calls(context.calls_spec, hand ++ draw, cxt_player.tile_behavior, []), fn {called_tile, call_choices} ->
          Enum.all?(call_choices, fn call_choice ->
            ...
            waits_after_call = Match.get_waits((hand ++ draw) -- call_tiles, calls ++ [call], win_definitions, cxt_player.tile_behavior)
      ...
      "has_hell_wait" ->
        ...
        ukeire = Match.get_waits_and_ukeire(cxt_player.hand, cxt_player.calls, wait_definitions, get_visible_tiles(state), cxt_player.tile_behavior)
      ...
      "amount_equals"       -> Actions.eval_expression(state, context, Enum.at(opts, 0, 0)) in Enum.map(Enum.drop(opts, 1), &Actions.eval_expression(state, context, &1))
      "amount_at_least"     -> Actions.eval_expression(state, context, Enum.at(opts, 0, 0)) >= Actions.eval_expression(state, context, Enum.at(opts, 1, 0))
      "amount_at_most"      -> Actions.eval_expression(state, context, Enum.at(opts, 0, 0)) <= Actions.eval_expression(state, context, Enum.at(opts, 1, 0))
      "amount_more_than"    -> Actions.eval_expression(state, context, Enum.at(opts, 0, 0)) > Actions.eval_expression(state, context, Enum.at(opts, 1, 0))
      "amount_less_than"    -> Actions.eval_expression(state, context, Enum.at(opts, 0, 0)) < Actions.eval_expression(state, context, Enum.at(opts, 1, 0))
      "genbutsu_shimocha"   ->

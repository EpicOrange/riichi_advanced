      ...
      Enum.any?(choice_actions, fn [action | _opts] -> action in ["call", "self_call", "upgrade_call", "flower", "draft_saki_card"] end) ->
        ...
        {state, call_choices} = cond do
          ...
          is_flower ->
            ...
            flowers_in_hand = Enum.filter(state.players[seat].hand ++ state.players[seat].draw, fn tile -> Utils.has_matching_tile?([tile], flowers) and is_playable?(state, seat, tile) end)

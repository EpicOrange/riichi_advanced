...
defmodule RiichiAdvanced.GameState.Marking do
  ...
  @special_keys [:done, :cancellable, :post_actions, :cancel_actions]
  @mark_targets %{
    "hand" => :hand,
    "call" => :calls,
    "aside" => :aside,
  ...
  def setup_marking(state, seat, to_mark, cancellable, post_actions, cancel_actions) do
    init = [done: false, cancellable: cancellable, post_actions: post_actions, cancel_actions: cancel_actions]
  ...
  def get_mark_infos(marked_objects, source) do
      ...
      case restriction do
        ...
        "match_hand_to_marked_call" ->
          ...
          |> Enum.all?(fn {call, _, _} ->
            call_tile = Utils.get_joker_meld_tile(call, state.players[marking_player].tile_behavior)
            Utils.same_tile(tile, Utils.strip_attrs(call_tile), state.players[marking_player].tile_behavior) end)
        "match_call_to_marked_hand" ->
          call_tile = Utils.get_joker_meld_tile(tile, state.players[marking_player].tile_behavior)
          state.marking[marking_player]
  ...
  def mark_tile(state, marking_player, seat, index, source) do
    if not Enum.empty?(state.marking[marking_player]) do
      ...
      not_already_marked = valid_mark_info_ix != nil and not Enum.any?(state.marking[marking_player] |> Enum.at(valid_mark_info_ix) |> elem(1) |> Map.get(:marked), fn {_, _, i} -> index == i end)
      if not_already_marked do

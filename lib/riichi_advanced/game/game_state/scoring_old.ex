  ...
  def calculate_delta_scores_for_single_winner(state, winner, collect_sticks) do
    ...
    delta_scores = for {payer, {yaku, yaku2, shares, mult, penalty}} <- liabilities, reduce: delta_scores do
      ...
      delta_scores ->
        ...
        basic_score = if total_shares > 0 do Utils.try_integer(basic_score * shares) else basic_score end
    ...
    delta_scores = if "use_arakawa_kei_scoring" in winner.player.status do
      ...
      waits = Match.get_waits_and_ukeire(winner.player.hand, winner.player.calls, win_definitions, visible_tiles, winner.tile_behavior)
        ...
        delta_scores = for {seat, player} <- state.players, reduce: delta_scores do
          delta_scores ->
            if delta_scores[seat] < 0 and "ezaki_hitomi_reflect" in player.status do
              ...
              win_definitions = Rules.translate_match_definitions(state.rules_ref, ["win"])
              waits = Match.get_waits(player.hand, player.calls, win_definitions, player.tile_behavior)

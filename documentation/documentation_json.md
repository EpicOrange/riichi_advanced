...
- `"restart"`: whenever the match definition encounters `"restart"`, it resets its current state to the initial hand and calls, as long as it has not already failed.
- `"fail"`: fails the match immediately.
...
- `["swap_out_joker", [joker1, joker2, ...], destination]`: Before running this action, you need to mark a tile in hand as well as one call containing the given `joker` tiles (e.g. `["1j"]`). This swaps the joker in the call with the tile in hand, moving the joker to `destination`, which is either `"draw"` (as is done in American and Malaysian mahjong) or `"call"` (as is done in Vietnamese mahjong).
...
- `{"name": "has_yaku", "opts": [yaku1, yaku2, ...]}`: Only works in `after_scoring`. Check if any of the given yaku names are present in the final score.
...
- `{"name": "is_atozuke", "opts": [match_spec, points, point_name, minipoints]}`: Using any wait as a discarded tile, the hand contains a wait that does not score enough `points` `point_name` and `minipoints`. Waits are any tile that can complete `match_spec` with the current hand. Example: `{"name": "is_atozuke", "opts": ["win", 1, "Han", 0]}`
...
- `"playable_tiles"`: selects tiles from the player's hand and draw that are discardable this turn.
...
For instance, this is how it's used in Riichi:
    ...
    [
      ...
      [[
        "ite",
        [{"name": "status", "opts": ["renchan"]}],
        [["change_dealership", "self"], ["add_honba", 1]],
        [[
          "when", [{"name": "amount_more_than", "opts": ["kyoku", 0]}],
          [
            ["set_counter", "delta_honba", "eval_expr", ["-", ["honba"]]],
            ["add_honba", "delta_honba"]
          ]
        ]]
      ]]
    ]

...
- `is_atozuke(match_spec, points, point_name, minipoints)`: Using any wait as a discarded tile, the hand contains a wait that does not score enough `points` `point_name` and `minipoints`. Waits are any tile that can complete `match_spec` with the current hand. Example: `is_atozuke("win", 1, "Han", 0)`
